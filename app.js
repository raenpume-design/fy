...
/* AYUDANTES DE FECHA Y FORMATO DE DINERO */
...
async function cargarDatos() {
    ...
    const { data: resTx } = await db.from('transacciones').select('*, cuentas(nombre), bolsillos(nombre)').order('fecha', { ascending: false }).limit(15);
    ...
    container.innerHTML = bolsillos.map(b => `
        <div class="item-row" style="background: #e0e7ff; display: flex; justify-content: space-between; align-items: center;">
            <div>
                ...
                <p style="font-size: 0.75rem; color: #4338ca;">Ahorrado: <b>${formatMoney(b.saldo_actual)}</b> de ${formatMoney(b.meta)}</p>
            </div>
            <div style="display: flex; gap: 0.3rem;">
                <button onclick="abrirModalSacarBolsillo('${b.id}', '${b.nombre}', ${b.saldo_actual})" style="background: #4338ca; color: white; border: none; padding: 0.3rem 0.5rem; border-radius: 0.4rem; font-size: 0.7rem; font-weight: 700; cursor: pointer;">💸 Sacar</button>
                <button onclick="eliminarBolsillo('${b.id}')" style="background: var(--danger); color: white; border: none; padding: 0.3rem 0.5rem; border-radius: 0.4rem; font-size: 0.7rem; font-weight: 700; cursor: pointer;">🗑️</button>
            </div>
        </div>
    `).join('');
}
...
/* GESTIÓN DE MODALES Y LIMPIEZA DE FORMULARIOS */
...
function abrirModalSacarBolsillo(id, nombre, saldoActual) {
    ...
    document.getElementById('sacar-resumen').innerText = `Saldo actual en "${nombre}": ${formatMoney(saldoActual)}`;
...
async function confirmarSacarBolsillo(e) {
    ...
    if (monto > saldoMax) {
        alert("El monto supera lo disponible en el bolsillo.");
        return;
    }
