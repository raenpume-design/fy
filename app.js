...
function poblarFiltroCuentas() {
    const select = document.getElementById('filter-cuenta');
    select.innerHTML = '<option value="">Todas</option>' + cuentas.map(c => `<option value="${c.id}">${c.nombre}</option>`).join('');
}
        ...
        return `
            <div class="item-row" style="background: #e0e7ff; display: flex; justify-content: space-between; align-items: center;">
                ...
                <div style="display: flex; gap: 0.25rem;">
                    ...
                    <button onclick="abrirModalSacarBolsillo('${b.id}', '${b.nombre}', ${b.saldo_actual})" style="background: #4338ca; color: white; border: none; padding: 0.3rem 0.4rem; border-radius: 0.4rem; font-size: 0.7rem; font-weight: 700; cursor: pointer;">💸 Sacar</button>
        ...
        return `
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.6rem 0; border-bottom: 1px solid var(--border); font-size: 0.85rem;">
                ...
                <div style="display: flex; align-items: center; gap: 0.4rem;">
                    ...
                    <button onclick="abrirModalEjecutarProyeccion('${p.id}', '${p.tipo}', ${p.monto}, '${p.concepto}', '${p.fecha}')" style="background: var(--success); color: white; border: none; padding: 0.25rem 0.4rem; border-radius: 0.4rem; font-size: 0.7rem; font-weight: 700; cursor: pointer;">✔ Hecho</button>
...
/* FILTRO DE MOVIMIENTOS EN MÓDULO COMPLETO */
function filtrarMovimientos() {
    const busqueda = document.getElementById('filter-search').value.toLowerCase();
    const tipo = document.getElementById('filter-tipo').value;
    const cuentaId = document.getElementById('filter-cuenta').value;
    ...
    const filtrados = todasLasTransacciones.filter(t => {
        ...
        const tipoMatch = !tipo || t.tipo === tipo;
        const cuentaMatch = !cuentaId || t.cuenta_id === cuentaId;
        
        let fechaMatch = true;
        if (t.fecha) {
            const txFecha = t.fecha.split('T')[0];
            if (desde && txFecha < desde) fechaMatch = false;
            if (hasta && txFecha > hasta) fechaMatch = false;
        }
    ...
    if (!filtrados.length) {
        container.innerHTML = '<p style="font-size: 0.8rem; color: var(--subtext);">No se encontraron movimientos con los filtros aplicados.</p>';
    } else {
...
async function eliminarTransaccion(id, tipo, monto, cuentaId, bolsilloId) {
    if (!confirm("¿Deseas eliminar este movimiento? Se reajustará el saldo de la cuenta automáticamente.")) return;
...
function setFiltroFecha(val, btn) {
    filtroFechaVal = val;
    btn.parentElement.querySelectorAll('.pill-btn').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
    filtrarMovimientos();
...
function setFiltroCuenta(val, btn) {
    filtroCuentaVal = val;
    btn.parentElement.querySelectorAll('.pill-btn').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
    filtrarMovimientos();
...
function renderFiltroCuentasPills() {
    const container = document.getElementById('filter-cuentas-pills');
    if (!container) return;
    let html = `<button class="pill-btn ${filtroCuentaVal === '' ? 'active' : ''}" onclick="setFiltroCuenta('', this)">Todas</button>`;
    html += cuentas.map(c => `<button class="pill-btn ${filtroCuentaVal === c.id ? 'active' : ''}" onclick="setFiltroCuenta('${c.id}', this)">${c.nombre}</button>`).join('');
